...
import { GetSprintsRequest, AppliedFilters } from './types.ts';
import { validateUUID, validatePagination, validateSprintStatuses } from './utils/validation.ts';
...
serve(async (req) => {
  ...
  try {
    ...
    const body: GetSprintsRequest = await req.json();
    ...
    if (status.length > 0) {
      const statusValidation = validateSprintStatuses(status);
    ...
    const cacheKey = generateCacheKey(body.projectId, status, includeStats, page, limit);
    ...
    const { sprints, totalCount } = await dbService.getSprints({
    ...
    const response = formatSuccessResponse(sprints, totalCount, page, limit, appliedFilters);
    ...
    console.error('Error in api-sprints-list:', error);
    const processingTime = Date.now() - startTime;
