...
import { fetchDeveloperMatrix, formatDeveloperMatrixForPrompt } from '../_shared/developer-matrix/service.ts';
import { RagContextService, loadRagContextConfig } from '../_shared/rag-context/index.ts';

const OPERATION = 'create-tasks';
...
const ragContextService = new RagContextService(supabase, OPENAI_API_KEY, `[${OPERATION}]`);
...
serve(async (req) => {
  ...
  try {
    ...
    // Inject developer matrix if project_id is provided
    if (body.project_id && userPrompt.includes('{{DEVELOPERS_MATRIX}}')) {
      console.log(`[${OPERATION}] Fetching developer matrix for project ${body.project_id}`);
      const developers = await fetchDeveloperMatrix(supabase, body.project_id);
      const formattedDevelopers = formatDeveloperMatrixForPrompt(developers);
      userPrompt = userPrompt.replace('{{DEVELOPERS_MATRIX}}', formattedDevelopers);
      console.log(`[${OPERATION}] Injected ${developers.length} developers into prompt`);
    }

    // Busca e injecao de contexto RAG
    const ragConfig = await loadRagContextConfig(supabase, `[${OPERATION}]`);
    const ragResult = await ragContextService.fetchContext(body.content, {
      projectId: body.project_id,
      creationType: 'tasks',
      config: ragConfig,
    });
    if (ragResult.used) {
      userPrompt = userPrompt + '\n\n' + ragResult.contextBlock + '\n\n' + ragResult.ragInstruction;
      console.log(`[${OPERATION}] RAG context injected: ${ragResult.resultsCount} results in ${ragResult.durationMs}ms`);
    } else {
      console.log(`[${OPERATION}] RAG context skipped (no results or disabled)`);
    }
    ...
    const interactionId = await aiInteractionService.createInteraction({
      ...
      request_parameters: {
        ...config,
        rag_used: ragResult.used,
        rag_results_count: ragResult.resultsCount,
        rag_duration_ms: ragResult.durationMs,
      },
