import { PerformanceTestRequest, Strategy } from '../types.ts';
...
/**
 * Validates the performance test request body
 * Throws ValidationError if validation fails
 */
export function validateTestRequest(body: unknown): asserts body is PerformanceTestRequest {
  ...
  if (request.locale !== undefined) {
    ...
    (request as PerformanceTestRequest).locale = DEFAULT_LOCALE;
  }
  ...
  if (request.timeout !== undefined) {
    ...
    // Set default if not provided
    (request as PerformanceTestRequest).timeout = DEFAULT_TIMEOUT;
  }
}
