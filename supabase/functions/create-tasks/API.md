# Create Document API Documentation
...
Generate (tasks) using OpenAI's GPT-4o model with conversation context support.
...
```bash
curl -X POST https://your-project.supabase.co/functions/v1/tasks \
...
- Metadata includes `project_id` and `operation: 'tasks'` for tracking
