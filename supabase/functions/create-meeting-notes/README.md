# Create Meeting Notes Edge Function

This Supabase Edge Function generates structured meeting notes (atas de reunião) using OpenAI's API with intelligent configuration management through the platform_settings system.
...
The `create-meeting-notes` function:
- Generates professional meeting notes from transcriptions or meeting content using OpenAI
...
- Outputs structured meeting notes in Brazilian Portuguese by default
...
AI configurations are stored in the `platform_settings` table with the key `ai-create-meeting-notes`.
...
```json
{
  ...
  "temperature": 0.6,
  "token_limit": 8000,
  "system_prompt": "Você é um especialista em gerar atas de reunião...",
...
The `user_prompt` supports the `{{content}}` placeholder, which is automatically replaced with the actual meeting transcription or content from the request body.

**Example:**
```json
{
  "user_prompt": "Gere uma ata de reunião baseada nesta transcrição:\n\n{{content}}\n\nInclua todas as seções padrão."
}
```

When a request comes in with `content: "Meeting transcription..."`, the system generates:
```
Gere uma ata de reunião baseada nesta transcrição:
...
curl -X POST 'https://your-project.supabase.co/functions/v1/create-meeting-notes' \
  ...
  -d '{
    "content": "Reunião de planejamento do sprint... [transcrição completa]",
    "project_id": "123e4567-e89b-12d3-a456-426614174000"
  }'
```

**Behavior:**
- Loads AI configuration from database (key: `ai-create-meeting-notes`)
...
curl -X POST 'https://your-project.supabase.co/functions/v1/create-meeting-notes' \
  ...
  -H 'Content-Type: application/json' \
  -d '{
    "content": "Reunião de planejamento do sprint...",
    "project_id": "123e4567-e89b-12d3-a456-426614174000",
    "model": "gpt-4o-mini",
    "temperature": 0.5,
    "system_prompt": "Você é um especialista em documentação ágil.",
    "user_prompt": "Crie uma ata de reunião detalhada:\n{{content}}"
  }'
```
...
curl -X POST 'https://your-project.supabase.co/functions/v1/create-meeting-notes' \
  ...
  -H 'Content-Type: application/json' \
  -d '{
    "content": "Adicione os itens de ação discutidos ao final da reunião",
    "project_id": "123e4567-e89b-12d3-a456-426614174000",
...
```sql
-- Insert default configuration for create-meeting-notes
...
```sql
SELECT * FROM platform_settings WHERE key = 'ai-create-meeting-notes';
```

2. **Check function logs:**
```bash
supabase functions logs create-meeting-notes
```
Look for log messages like:
- `[create-meeting-notes] Loaded configuration from database: ai-create-meeting-notes`
- `[create-meeting-notes] Using database prompts for key: ai-create-meeting-notes`
...
```bash
supabase functions serve create-meeting-notes --env-file .env
```

5. **Test the function:**
```bash
curl -X POST 'http://localhost:54321/functions/v1/create-meeting-notes' \
  -H 'Authorization: Bearer YOUR_LOCAL_ANON_KEY' \
  -H 'Content-Type: application/json' \
  -d '{
    "content": "Test meeting transcription",
    "project_id": "test-id"
  }'
```
...
```bash
# Real-time logs
supabase functions logs create-meeting-notes --follow
...
2. Review function logs: `supabase functions logs create-meeting-notes`
