...
export class PageSpeedClient {
  ...
  /**
   * Runs a PageSpeed Insights performance test for the given URL
  ...
  async runTest(
    ...
    // NOTE: API requires category=PERFORMANCE (uppercase) and strategy in specific format
  ...
  private validateResponse(data: any): void {
    ...
    if (!data.lighthouseResult.categories?.performance) {
      throw new ApiError(
        'Invalid response structure: missing performance category',
        500,
        false
      );
    }
