...
serve(async (req) => {
  ...
  try {
    ...
    try {
      ...
      console.error('[api-projects] JSON parse error:', parseError);
    ...
    switch (action) {
      case 'get':
        return await handleGetRequest(body as GetProjectRequest, requestId, startTime);

      case 'list':
        return await handleListRequest(body as ListProjectsRequest, requestId, startTime);

      case 'update':
        return await handleUpdateRequest(body as UpdateProjectRequest, requestId, startTime);

      case 'create':
        return await handleCreateRequest(body as CreateProjectRequest, requestId, startTime);

      default:
        // If no action specified, treat as create request
        return await handleCreateRequest(body as CreateProjectRequest, requestId, startTime);
    }
  } catch (error) {
