# Create PRD Edge Function

This Supabase Edge Function generates Product Requirement Documents (PRDs) using OpenAI's API with intelligent configuration management through the platform_settings system.
...
The `create-prd` function:
- Generates PRDs from user-provided content using OpenAI
...
AI configurations are stored in the `platform_settings` table with the key `ai-create-prd`.
...
```json
{
  ...
  "user_prompt": "Create a comprehensive PRD for:\n{{content}}"
}
```
...
```json
{
  "user_prompt": "Create a PRD based on this input:\n\n{{content}}\n\nInclude all standard sections."
}
```

When a request comes in with `content: "Build a mobile app for task management"`, the system generates:
```
Create a PRD based on this input:
...
```bash
curl -X POST 'https://your-project.supabase.co/functions/v1/create-prd' \
  -H 'Authorization: Bearer YOUR_ANON_KEY' \
  -H 'Content-Type: application/json' \
  -d '{
    "content": "Build a mobile app for task management with real-time sync",
    "project_id": "123e4567-e89b-12d3-a456-426614174000"
  }'
```

**Behavior:**
- Loads AI configuration from database (key: `ai-create-prd`)
...
curl -X POST 'https://your-project.supabase.co/functions/v1/create-prd' \
  ...
  -d '{
    ...
    "model": "gpt-4o-mini",
    "temperature": 0.5,
    "system_prompt": "You are a technical product manager focused on mobile apps.",
    "user_prompt": "Create a detailed PRD:\n{{content}}"
  }'
```
...
curl -X POST 'https://your-project.supabase.co/functions/v1/create-prd' \
  ...
  -H 'Content-Type: application/json' \
  -d '{
    "content": "Add offline mode and dark theme",
    "project_id": "123e4567-e89b-12d3-a456-426614174000",
...
```sql
-- Insert default configuration for create-prd
...
VALUES (
  ...
  '{
    ...
    "temperature": 0.7,
    "token_limit": 4096,
    "system_prompt": "You are an expert product manager who creates comprehensive Product Requirement Documents.",
    "user_prompt": "Create a detailed PRD based on the following:\n\n{{content}}"
  }'::jsonb,
  'AI configuration for PRD generation'
)
...
```sql
SELECT * FROM platform_settings WHERE key = 'ai-create-prd';
```
...
- `[create-prd] Loaded configuration from database: ai-create-prd`
- `[create-prd] Using database prompts for key: ai-create-prd`
...
```bash
supabase functions serve create-prd --env-file .env
```
...
```bash
# Real-time logs
supabase functions logs create-prd --follow
...
2. Review function logs: `supabase functions logs create-prd`
