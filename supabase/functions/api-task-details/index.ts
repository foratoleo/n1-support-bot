...
import { GetTaskDetailsRequest } from './types.ts';
...
serve(async (req) => {
  ...
  try {
    ...
    const body: GetTaskDetailsRequest = await req.json();
    ...
    const task = await dbService.getTaskDetails(body.projectId, body.taskId);
    ...
    return new Response(
      JSON.stringify(formatSuccessResponse(task)),
      {
        status: 200,
        headers: {
          ...corsHeaders,
