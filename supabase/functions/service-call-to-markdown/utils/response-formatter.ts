...
import { ValidationError, NotFoundError, DatabaseError, UnsupportedServiceError } from './validation.ts';
...
export function formatErrorResponse(error: Error, statusCode?: number): Response {
  ...
  if (!statusCode) {
    if (error instanceof ValidationError) {
      ...
      status = 404;
    } else if (error instanceof UnsupportedServiceError) {
      status = 400;
  ...
  if (error instanceof ValidationError) {
    ...
    errorBody.details = {
      ...
      serviceCategory: error.serviceCategory,
    };
  }

  // Log error for monitoring
  logError(error, { statusCode: status });
