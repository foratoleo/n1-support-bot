...
export class ProgressTracker {
  ...
  private syncDirection: 'dr_to_jira' | 'jira_to_dr';

  constructor(
    dbService: JiraDbService,
    flushInterval: number = 1000, // Flush updates every second
    syncDirection: 'dr_to_jira' | 'jira_to_dr' = 'jira_to_dr' // Default for this endpoint
  ) {
  ...
  private async flushUpdates(): Promise<void> {
    if (this.updateQueue.size === 0) {
      // Auto-stop timer when no active operations to prevent resource leak
  ...
  private async persistProgress(progress: SyncProgressData): Promise<void> {
    ...
    // Map 'cancelled' status to 'error' for database compatibility
    const dbStatus = progress.status === 'processing' ? 'pending' :
                     progress.status === 'completed' ? 'success' :
                     progress.status === 'cancelled' ? 'error' : 'error';

    await this.dbService.createSyncLog({
      project_id: progress.projectId,
      operation: progress.operation,
      direction: this.syncDirection, // Use configured direction instead of hardcoded value
      status: dbStatus,
      metadata: {
