...
serve(async (req) => {
  ...
  try {
    ...
    try {
      ...
      console.error('[api-sprints] JSON parse error:', parseError);
    ...
    switch (action) {
      case 'get':
        return await handleGetRequest(body as GetSprintRequest, requestId, startTime);

      case 'list':
        return await handleListRequest(body as ListSprintsRequest, requestId, startTime);

      case 'update':
        return await handleUpdateRequest(body as UpdateSprintRequest, requestId, startTime);

      case 'create':
        return await handleSingleRequest(body as CreateSprintRequest, requestId, startTime);

      default:
        return await handleSingleRequest(body as CreateSprintRequest, requestId, startTime);
    }
  } catch (error) {
    console.error('[api-sprints] Error:', error);
