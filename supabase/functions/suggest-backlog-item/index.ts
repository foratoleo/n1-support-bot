...
const OPERATION = 'suggest-backlog-item';
...
serve(async (req) => {
  ...
  try {
    ...
    if (!methodValidation.valid) {
      console.error(`[${OPERATION}] Method validation failed:`, methodValidation.error);
    ...
    if (!bodyValidation.valid) {
      console.error(`[${OPERATION}] Body validation failed:`, bodyValidation.error);
    ...
    try {
      ...
      if (!document) {
        console.error(`[${OPERATION}] Failed to extract document from response`);
      ...
      const successData = formatSuccessResponse(document, resp.id);
      return createResponse(successData, 200);
    } catch (genError) {
      console.error(`[${OPERATION}] Generation error:`, (genError as Error)?.message);
    ...
    console.error(`[${OPERATION}] Edge function error:`, (error as Error)?.message);
