...
const OPERATION = 'create-user-story';
...
serve(async (req) => {
  ...
  try {
    ...
    const { config } = await loadConfiguration(platformSettingsService, CONFIG_KEY as AIConfigurationKey, OPENAI_CONFIG, {
      model: body.model,
    ...
    const { systemPrompt, userPrompt } = buildPrompts(body.content, DEFAULT_SYSTEM_PROMPT, DEFAULT_USER_PROMPT, dbConfig || {}, {
