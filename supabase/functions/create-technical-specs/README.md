# Create Technical Specs Edge Function

This Supabase Edge Function generates comprehensive technical specifications using OpenAI's API with intelligent configuration management through the platform_settings system.
...
The `create-technical-specs` function:
- Generates detailed technical specifications from user-provided content using OpenAI
...
- Outputs technical specifications in Brazilian Portuguese by default
- Covers architecture, design, APIs, data models, security, and performance
...
AI configurations are stored in the `platform_settings` table with the key `ai-create-technical-specs`.
...
```json
{
  ...
  "temperature": 0.5,
  "token_limit": 8000,
  "system_prompt": "You are a specialized Technical Specification generator...",
  "user_prompt": "Generate a detailed technical specification based on:\n{{content}}"
}
```

**Key Fields:**
- `model`: OpenAI model to use (e.g., "gpt-4o", "gpt-4o-mini")
- `temperature`: Randomness/creativity (0.0 to 2.0) - moderate for technical specs (0.5 recommended)
...
```json
{
  "user_prompt": "Create comprehensive technical specification for:\n\n{{content}}\n\nInclude architecture, APIs, and data models."
}
```

When a request comes in with `content: "Real-time chat system with WebSocket support"`, the system generates:
```
Create comprehensive technical specification for:
...
curl -X POST 'https://your-project.supabase.co/functions/v1/create-technical-specs' \
  ...
  -d '{
    "content": "Real-time chat system with WebSocket support and message persistence",
    "project_id": "123e4567-e89b-12d3-a456-426614174000"
  }'
```

**Behavior:**
- Loads AI configuration from database (key: `ai-create-technical-specs`)
...
curl -X POST 'https://your-project.supabase.co/functions/v1/create-technical-specs' \
  ...
  -H 'Content-Type: application/json' \
  -d '{
    "content": "Real-time chat system with WebSocket support",
    "project_id": "123e4567-e89b-12d3-a456-426614174000",
    "model": "gpt-4o-mini",
    "temperature": 0.4,
    "system_prompt": "You are a senior software architect specializing in real-time systems.",
    "user_prompt": "Create detailed technical architecture for:\n{{content}}"
  }'
```
...
curl -X POST 'https://your-project.supabase.co/functions/v1/create-technical-specs' \
  ...
  -H 'Content-Type: application/json' \
  -d '{
    "content": "Add specifications for file sharing and video call features",
    "project_id": "123e4567-e89b-12d3-a456-426614174000",
...
```sql
-- Insert default configuration for create-technical-specs
...
VALUES (
  ...
  '{
    ...
    "temperature": 0.5,
    "token_limit": 8000,
    "system_prompt": "You are a specialized Technical Specification generator for software development.",
    "user_prompt": "Generate a detailed technical specification based on the following content:\n\n{{content}}"
  }'::jsonb,
  'AI configuration for technical specifications generation'
)
...
```sql
SELECT * FROM platform_settings WHERE key = 'ai-create-technical-specs';
```

2. **Check function logs:**
```bash
supabase functions logs create-technical-specs
```
Look for log messages like:
- `[create-technical-specs] Loaded configuration from database: ai-create-technical-specs`
- `[create-technical-specs] Using database prompts for key: ai-create-technical-specs`
...
WHERE key = 'ai-create-technical-specs';
```
...
```bash
supabase functions serve create-technical-specs --env-file .env
```

5. **Test the function:**
```bash
curl -X POST 'http://localhost:54321/functions/v1/create-technical-specs' \
...
```bash
# Real-time logs
supabase functions logs create-technical-specs --follow
...
- [Create Test Cases Function](../create-test-cases/README.md)

## Support

For issues or questions:
1. Check this troubleshooting guide
2. Review function logs: `supabase functions logs create-technical-specs`
